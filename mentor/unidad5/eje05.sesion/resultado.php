<?php
	...
	{	
		...
		// Creamos variables de sesión nombre y usuario del formulario
		...
		// Variables de sesión: hora, contador y pagina ($_SERVER['PHP_SELF'])
		...
		$resultadoStr.= "<TABLE border=1 WIDTH=300><TR><TD>
			...
			Página web en la que ha entrado : <P><B>".$_SESSION['pagina']."</B><P>";

		// Mostramos la información de la sesión codificada 
		$cadena=session_encode();
		$resultadoStr.= "La función <B>session_encode()</B> devuelve la cadena: <P><B>$cadena</B><P>";
		
		$resultadoStr.=  "Ahora borramos el contenido de la variable 'usuario' y observamos como varía ";			
		...
		$cadena=session_encode();
		$resultadoStr.= "la función <B>session_encode()</B>. Al final de la cadena ya no aparece usuario: <P><B>$cadena</B><P>";
...
<head>
	...
	<title>Ejemplo 5 - Unidad 5</title>
</head>
<body>
